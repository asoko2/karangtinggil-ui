<template>
    <v-col cols="12">
        <v-btn color="secondary" to="/admin/permohonan-surat/sku/tambah">
            <v-icon>mdi-plus-thick</v-icon>Tambah SKU
        </v-btn>
        <v-card>
            <v-toolbar flat>
                ...
                <v-col cols="12">
                    <v-data-table :headers="headers" :items="skus" disable-pagination :options.sync="options"
                        ...
                        <template v-slot:[`item.actions`]="{ item }">
                            <router-link :to="'/admin/permohonan-surat/sku/' + item.id" class="primary--text"
